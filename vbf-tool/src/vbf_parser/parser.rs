...
impl VbfFt {
    pub fn new(vbb_path: &str) -> io::Result<()> {
        ...
        vbf_inst.vbt_info.blk[0].start_addr =
            u32::from_str_radix(&vbf_inst.script.image_offset.value.literal().unwrap(), 16)
                .unwrap();
    ...
    pub fn dump(&mut self, fp: &mut fs::File) -> io::Result<()> {
        ...
        fp.write(b"    //*                      Freetech.co\r\n")
            .unwrap();
        fp.write(b"    //*\r\n").unwrap();
        fp.write(b"    //*     This file is generated by vbf tool    ver.  0.1.2\r\n")
            .unwrap();
        fp.write(b"    //*\r\n").unwrap();
        ...
        // maybe add file description info here
        ...
        fp.write(b"    // Volvo software part type: EXE||SBL||CAL\r\n")
            .unwrap();
        fp.write_fmt(format_args!(
            "       sw_part_type = {};\r\n",
        ...
        if vbt_enable {
            fp.write_fmt(format_args!(
                "                 {{ 0x{:08X}, 0x{:08X} }}\r\n",
                u32::from_str_radix(&self.script.vbt_addr.value.literal().unwrap(), 16).unwrap(),
        ...
        if vbt_enable {
            ...
            let vbt_address: [u8; 4] = unsafe {
                transmute(
                    u32::from_str_radix(&self.script.vbt_addr.value.literal().unwrap(), 16)
                        .unwrap()
                        .to_be(),
                )
            };
...
#[derive(Debug)]
struct ScriptsT {
    description: Item,  // file info descriptions
